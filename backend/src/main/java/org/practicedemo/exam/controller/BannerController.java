package org.practicedemo.exam.controller;
...
import org.practicedemo.exam.entity.Banner;
import org.practicedemo.exam.service.BannerService;
...
public class BannerController {
    ...
    public Result<List<Banner>> getActiveBanners() {
        ...
        queryWrapper.eq(Banner::getIsActive, 1);
        //2.排序
        ...
        //4.进行结果拼接
        return Result.success(list,"查询前台需要的激活状态轮播信息成功！");
    }
    ...
    public Result<List<Banner>> getAllBanners() {
        //1.拼接条件，进行排序
        LambdaQueryWrapper<Banner> queryWrapper = new LambdaQueryWrapper<>();
        queryWrapper.orderByAsc(Banner::getSortOrder);//根据sort字段升序排序
        //2.执行查询
        List<Banner> bannerList = bannerService.list(queryWrapper);
        log.info("查询所有轮播图业务执行成功！结果为：{}",bannerList);
        //3.进行结果拼接
        return Result.success(bannerList,"查询所有轮播图成功！");
    }
    ...
    public Result<String> toggleBannerStatus(
        ...
        //执行更新
        boolean updated = bannerService.update(updateWrapper);
        if (updated) {
            log.info("切换轮播图状态成功！切换id为：{}的轮播图状态为：{}",id,isActive);
            return Result.success("轮播图状态切换成功！");
        }
        log.info("切换轮播图状态失败！计划切换id为：{}的轮播图状态为：{}",id,isActive);
        return Result.error("轮播图状态切换失败！");
    }
} 