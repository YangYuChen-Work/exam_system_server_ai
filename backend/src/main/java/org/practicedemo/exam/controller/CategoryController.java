...
public class CategoryController {
    ...
    @Autowired
    private CategoryService categoryService;  // 自动注入分类服务
    ...
    public Result<List<Category>> getCategories() {
        ...
        log.info("获取分类列表成功！ 分类数量：{}", categories.size());
    ...
    public Result<List<Category>> getCategoryTree() {
        List<Category> categories = categoryService.getCategoriesTree();
        log.info("获取分类树形结构成功！ 分类数量：{}", categories.size());
        return Result.success(categories);
    }
    ...
    public Result<Void> addCategory(@RequestBody Category category) {
        try {
            ...
            log.info("添加分类成功！ 分类名称：{}", category.getName());
            return Result.success(null);
        } catch (Exception e) {
            log.error("添加分类失败！ 分类名称：{}，错误信息：{}", category.getName(), e.getMessage());
    ...
    public Result<Void> updateCategory(@RequestBody Category category) throws Exception {
        ...
        try {
            ...
            log.error("更新分类失败！ 分类ID：{}，错误信息：{}", category.getId(), e.getMessage());
            return Result.error(e.getMessage());
        }
    }
