...
public class QuestionController {
     ...
     * @param questionImportVo 题目筛选条件对象，包含分类、难度、题型、关键词等字段
     * @return 封装的分页查询结果，包含题目列表和分页信息
     */

    @Autowired
    private QuestionService questionService;  // 注入题目服务层依赖
    ...
    public Result<Page<Question>> getQuestionList(
        ...
        Page<Question> pageList = new Page<>(page, size);
        // 调用服务层方法执行分页查询，根据筛选条件构建查询条件
        questionService.customPageService(pageList, questionPageVo);
        // 日志记录，记录分页查询参数和筛选条件
        log.info("分页查询题目列表，参数：page={}, size={}, queryVo={}", page, size, questionPageVo);
        // 返回统一格式的成功响应，包含分页数据
